...
namespace GProject
{
    [Activity(Label = "Empty Places Based Search", ConfigurationChanges = Android.Content.PM.ConfigChanges.Orientation | Android.Content.PM.ConfigChanges.ScreenSize)]

    class TimeTableEmptyR : Activity
    {
        private Spinner combo_DaysEmpty, combo_TimesEmpty, combo_DurationEmpty;
        private int ptime;
        private GridView grid_Empty;
        private ArrayAdapter adapter;
        private string stime, tday;
        private ArrayList adays,atimes,adurations;
        private string[,] emptyclasses;
        protected override void OnCreate(Bundle savedInstanceState)
        {
            base.OnCreate(savedInstanceState);
            SetContentView(Resource.Layout.Time_TableEmptyR);

            combo_TimesEmpty = FindViewById<Spinner>(Resource.Id.comboTimesEmptyR);
            combo_DaysEmpty = FindViewById<Spinner>(Resource.Id.comboDaysEmptyR);
            combo_DurationEmpty = FindViewById<Spinner>(Resource.Id.comboDurationEmptyR);
            grid_Empty = FindViewById<GridView>(Resource.Id.gridEmptyR);
            combos();
            ...
            combo_TimesEmpty.ItemSelected += Combo_TimesEmpty_ItemSelected;
        ...
        private void Combo_TimesEmpty_ItemSelected(object sender, AdapterView.ItemSelectedEventArgs e)
        {
            grid_Empty.Adapter = null;

            picktime();
            tday = combo_DaysEmpty.SelectedItem.ToString().Substring(0, 3);

            WebClient client = new WebClient();
            Uri url = new Uri("http://n00ne.xyz/xamarin_emptyR.php");
